...
const Cart = () => {
  ...
  const orderRegister = () => {
    axios
      .post(`http://localhost:3000/order/add`, { userId, productsList: carts })
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3000/carts/getcartbyuser/${userId}`)
      .then((response) => {
        setCarts(response.data);
      })
      .catch((err) => {
  ...
  const handleQuantityUpdate = (cartId: number, newQuantity: number) => {
    axios
      .put(`http://localhost:3000/carts/update/${cartId}`, {
      ...
      <div className="cart-items">
        ...
        {carts.map((cart) => (
          ...
          >
            <img
              src={cart.image}
              alt={cart.productId.toString()}
              className="product-image"
            />
            <span style={{ position: "absolute", marginLeft: "230px" }}>
              $ {cart.price}
            </span>
            <div
              style={{
            ...
            >
              <b
                ...
                onClick={() => handleQuantityUpdate(cart.id, cart.quantity - 1)}
              >
                -
              </b>
              <b
